...
import bibliothek.gui.dock.station.toolbar.ContainerLineConverterCallback;
import bibliothek.gui.dock.station.toolbar.ContainerLineDropInfo;
import bibliothek.gui.dock.station.toolbar.ContainerLineProperty;
import bibliothek.gui.dock.station.toolbar.ContainerLineStationFactory;
import bibliothek.gui.dock.station.toolbar.DefaultContainerLineConverter;
import bibliothek.gui.dock.station.toolbar.ContainerLineConverter;
...
public class ContainerLineStation extends AbstractDockableStation implements
	...
	/**
	 * Creates a {@link ContainerLineConverter} which will be used for one call.
	 * 
	 * @return the new converter, not <code>null</code>
	 */
	protected ContainerLineConverter createConverter(){
		return new DefaultContainerLineConverter();
	}
	...
	public StationDropOperation prepareDrop( int mouseX, int mouseY,
		...
		if (this.accept(dockable) & dockable.accept(this)){
			...
			ContainerLineDropInfo result = new ContainerLineDropInfo(dockable,
					this, dockables, mouseX, mouseY){
				@Override
				public void execute(){
					drop(this);
				}
				...
				public void destroy(){
					...
					ContainerLineStation.this.indexBeneathMouse = -1;
					ContainerLineStation.this.sideAboveMouse = null;
					ContainerLineStation.this.prepareDropDraw = false;
					ContainerLineStation.this.mainPanel.repaint();
				...
				public void draw(){
					...
					ContainerLineStation.this.indexBeneathMouse = indexOf(getDockableBeneathMouse());
					System.out.println("draw: "
							+ ContainerLineStation.this.indexBeneathMouse);
					ContainerLineStation.this.prepareDropDraw = true;
					ContainerLineStation.this.sideAboveMouse = this
	...
	private void drop( ContainerLineDropInfo dropInfo ){
	...
	public boolean drop( Dockable dockable, DockableProperty property ){
		if (property instanceof ContainerLineProperty){
			ContainerLineProperty toolbar = (ContainerLineProperty) property;
	...
	public String getFactoryID(){
		return ContainerLineStationFactory.ID;
	}
	...
	protected void callDockUiUpdateTheme() throws IOException{
		DockUI.updateTheme(this, new ContainerLineStationFactory());
	}
		...
		protected void paintOverlay( Graphics g ){
			...
			if (prepareDropDraw){
				if (indexBeneathMouse != -1){
					...
					switch (ContainerLineStation.this.getOrientation()) {
					case VERTICAL:
						int y;
