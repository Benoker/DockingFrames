...
/**
 * This {@link ExpandAction} is used for items that can switch only between
 * two of the {@link ExpandedState}s.
 * @author Benjamin Sigg
 */
public class SwitchExpandAction extends ExpandAction{
	public SwitchExpandAction( DockController controller ){
		super( controller, Action.SMALLER, Action.SMALLEST, Action.LARGER, Action.LARGEST );
		
		setGenerator( new GroupKeyGenerator<Action>(){
			@Override
			public Action generateKey( Dockable dockable ){
				boolean shrunk = getStrategy().isEnabled( dockable, ExpandedState.SHRUNK );
				boolean expanded = getStrategy().isEnabled( dockable, ExpandedState.EXPANDED );
