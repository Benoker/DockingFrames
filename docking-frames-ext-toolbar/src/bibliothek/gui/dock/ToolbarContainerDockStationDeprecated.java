 ...
 */
public class ToolbarContainerDockStationDeprecated extends AbstractDockableStation
	 ...
	 */
	public ToolbarContainerDockStationDeprecated( Orientation orientation ){
		...
		displayerFactory = new DefaultDisplayerFactoryValue(
				ThemeManager.DISPLAYER_FACTORY + ".toolbar.container",
				this);
		...
		DockableDisplayerListener listener = new DockableDisplayerListener(){
			...
			public void discard( DockableDisplayer displayer ){
				ToolbarContainerDockStationDeprecated.this.discard(displayer);
			}
		};
				...
				new ToolbarContainerConverterCallback(){
					...
					public StationChildHandle wrap( Dockable dockable ){
						return new StationChildHandle(
								ToolbarContainerDockStationDeprecated.this,
					...
					public void setDockables(
						...
						ToolbarContainerDockStationDeprecated.this.setDockables(list,
								false);
					}
	...
	public StationDropOperation prepareDrop( int mouseX, int mouseY,
		...
		if (this.accept(dockable) & dockable.accept(this)){
			...
			ToolbarContainerDropInfoDeprecated result = new ToolbarContainerDropInfoDeprecated(
				...
				public void destroy(){
					...
					ToolbarContainerDockStationDeprecated.this.indexBeneathMouse = -1;
					ToolbarContainerDockStationDeprecated.this.sideAboveMouse = null;
					ToolbarContainerDockStationDeprecated.this.prepareDropDraw = false;
					ToolbarContainerDockStationDeprecated.this.mainPanel.repaint();
				...
				public void draw(){
					...
					ToolbarContainerDockStationDeprecated.this.indexBeneathMouse = indexOf(getDockableBeneathMouse());
					System.out
							.println("draw: "
									+ ToolbarContainerDockStationDeprecated.this.indexBeneathMouse);
					ToolbarContainerDockStationDeprecated.this.prepareDropDraw = true;
					ToolbarContainerDockStationDeprecated.this.sideAboveMouse = this
	...
	public void setOrientation( Orientation orientation ){
		...
		for (int i = 0; i < dockables.dockables().size(); i++){
			if (dockables.dockables().get(i).getDockable() instanceof OrientedDockStation){
				...
				list.add((dockables.dockables().get(i)
						.getDockable()));
			}
		}
	...
	private void remove( Dockable dockable ){
		...
		try{
			...
			getContainerPanel()
					.remove(childHandle.getDisplayer().getComponent());
			childHandle.destroy();
		...
		protected void paintOverlay( Graphics g ){
			...
			if (prepareDropDraw){
				if (indexBeneathMouse != -1){
					...
					switch (ToolbarContainerDockStationDeprecated.this.getOrientation()) {
					case VERTICAL:
						int y;
