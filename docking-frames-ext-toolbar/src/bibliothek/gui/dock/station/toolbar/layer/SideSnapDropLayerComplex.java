	...
	public boolean contains( int x, int y ){
		...
		if( station.columnCount() == 0 ) {
			...
			for( int columnIndex = 0; columnIndex < station.columnCount(); columnIndex++ ) {
				...
				Rectangle lastBounds = new Rectangle( upperleft.x, upperleft.y, lastBoundsDraft.width, lastBoundsDraft.height );

				// the bounds of the column increased with the snap size
				Rectangle deducted = new Rectangle( firstBounds.x - size, firstBounds.y - size, ((int) lastBounds.getMaxX() - firstBounds.x) + (size * 2),
						((int) lastBounds.getMaxY() - firstBounds.y) + (size * 2) );
	...
	private boolean isComponentContain( int x, int y ){
		if( station.columnCount() == 0 ) {
			...
			for( int columnIndex = 0; columnIndex < station.columnCount(); columnIndex++ ) {
				...
				// the bounds of the column increased with the snap size
				Rectangle deducted = new Rectangle( firstBounds.x, firstBounds.y, ((int) lastBounds.getMaxX() - firstBounds.x),
