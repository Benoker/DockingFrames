...
public class ToolbarDockStationMerger extends AbstractToolbarMerger {
	...
	public void merge( StationDropOperation operation, DockStation parent, DockStation child ){
		...
		int increment = 0;
		if( operationToolbar.getSideDockableBeneathMouse() == Position.SOUTH || operationToolbar.getSideDockableBeneathMouse() == Position.EAST ) {
			increment++;
		}
		int dropIndex = station.indexOf( operationToolbar.getDockableBeneathMouse() ) + increment;
		for( int i = 0; i < count; i++ ) {
