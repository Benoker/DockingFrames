 ...
 */
public abstract class ToolbarComplexDropInfo<S extends OrientedDockStation> implements StationDropOperation {
	 ...
	 */
	public ToolbarComplexDropInfo( Dockable dockable, S stationHost, int mouseX, int mouseY ){
		System.out.println( this.toString() + "## new ToolbarComplexDropInfo ## " );
	...
	private Dockable computeDockableBeneathMouse(){
		...
		Point middleCoordinate = new Point( (int) stationHost.getDockable( 0 ).getComponent().getBounds().getCenterX(), (int) stationHost.getDockable( 0 )
				.getComponent().getBounds().getCenterY() );
		...
		switch( orientedStation.getOrientation() ){
			case VERTICAL:
				...
				for( int i = 1; i < dockableCount; i++ ) {
					middleCoordinate = new Point( (int) stationHost.getDockable( i ).getComponent().getBounds().getCenterX(), (int) stationHost.getDockable( i )
							.getComponent().getBounds().getCenterY() );
	...
	public String toSummaryString(){
		...
		return "	=> Drag dockable: " + getItem() + ln + "	=> Station target: " + getTarget() + ln + "	=> Dockable beneath mouse:" + getDockableBeneathMouse()
				+ ln + "	=> Closest side:" + this.getSideDockableBeneathMouse() + ln + "	=> Drag dockable VS dockable beneath mouse:"
