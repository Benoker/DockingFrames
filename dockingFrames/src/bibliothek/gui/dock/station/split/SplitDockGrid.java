...
/**
 * A class that provides a grid for {@link Dockable Dockables}. The grid can be transformed into a {@link SplitDockTree} which has values that would layout the components as they are in the grid. The algorithms used in this class can handle overlapping elements and holes, however results are much better if there are no disturbances in the grid.<br>
...
public class SplitDockGrid {
	 ...
	 * The argument <code>layout</code> is a string divided by newline <code>"\n"</code>. Every line represents a y-coordinate, the position of a character in a line represents a x-coordinate. The minimal and the maximal x- and y-coordinates for a character is searched, and used to call {@link #addDockable(double, double, double, double, Dockable[]) addDockable}, where the <code>Dockable</code>-array is taken from the {@link Map} <code>dockables</code>.
	 * 
	 * @param layout
	 *            the layout, a string divided by newlines
	 * @param dockables
	 *            the Dockables to add, only entries whose character is in the String <code>layout</code>.
	 */
	...
	/**
	 * Adds <code>dockable</code> to the grid. The coordinates are not absolute, only the relative location and size matters. If there are already <code>dockables</code> at the exact same location, then the <code>dockables</code> are stacked.
	 * 
	 * @param x
	 *            the x-coordinate
	 * @param y
	 *            the y-coordinate
	 * @param width
	 *            the width, more than 0
	 * @param height
	 *            the height, more than 0
	 * @param dockables
	 *            the Dockables to add
	 */
	...
	/**
	 * Marks <code>dockable</code> as selected in the stack of elements that are on position <code>x, y, width, height</code>. This method requires that {@link #addDockable(double, double, double, double, Dockable...) add} was called with the exact same coordinates and with <code>dockable</code>.
	 * 
	 * @param x
	 *            the x coordinate
	 * @param y
	 ...
	 * @param dockable
	 *            the element to select, not <code>null</code>
	 * @throws IllegalArgumentException
	 *             if <code>width</code> or <code>height</code> are below 0, if <code>dockable</code> is <code>null</code>, if {@link #addDockable(double, double, double, double, Dockable...) add} was never called with the arguments
	 */
	 ...
	 * @param b
	 *            the second node
	 * @return how likely the two nodes can be merged, a small result indicates that merging would be a good idea.
	 */
	...
	/**
	 * Searches the line that divides the rectangle <code>x, y, width, height</code> best.
	 * 
	 * @param x
	 ...
	 * @param horizontal
	 *            whether the line should be horizontal or not
	 * @param split
	 *            the preferred value of {@link Line#alpha}.
	...
	/**
	 * Used by {@link #diff(SplitDockGrid.Node, SplitDockGrid.Node) diff} to calculate add a penalty if a line hits a rectangle.
	 * 
	 * @param x
	 *            the x-coordinate of the rectangle
	 * @param y
	...
	protected static class Node {
		...
		private void toString(StringBuilder builder, int offset) {
			...
			if( dockables != null ){
				for( Dockable dockable : dockables ){
					builder.append( dockable.getClass().getSimpleName() );
					builder.append( "[");
					builder.append( dockable.getTitleText() );
					builder.append( "] " );
				}
			}
			...
			builder.append("\n");
			if (childA != null) {
				childA.toString(builder, offset + 1);
			}
			if (childB != null) {
				childB.toString(builder, offset + 1);
			}
		}
	}
}
