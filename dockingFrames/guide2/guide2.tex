...
Any module that wants to show a \src{DockTitle} needs to specify what kind of title it shows. The module needs an instance of \src{DockTitleVersion} to do so. A \src{DockTitleVersion} can be created by the \src{DockTitleManager}. When a module asks for a \src{DockTitleVersion} it has to hand over a \src{DockTitleFactory} to the manager. This factory becomes the default factory and will be used to create titles (unless someone replaces the default factory).
