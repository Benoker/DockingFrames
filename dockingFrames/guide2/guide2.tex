...
\src{DockingFrames} consists of two libraries, \src{Core} and \src{Common}. \src{Core} provides the basic functionality of the framework. It introduces a lot of features, but does not connect them. \src{Core} can be used without \src{Common}, but it gets a lot easier if both libraries are used together.

This document covers only \src{Core}, \src{Common} has its own guide. Not all the details of \src{Core} are described, but this document gives a nice oversight of the more important aspects.
...
\src{DockingFrames} distinguishes between local and global layout information. Local information only describes the relationship between one \src{Dockable} and its parent, global information describes whole trees of elements. There are no algorithms which recreate a global layout only from local information, but there are also no algorithms which can place a \src{Dockable} in an existing tree using global information. So both kinds of data have their use.
...
This chapter explains the mechanism of creating and managing \src{DockTitle}s. 
...
The \textit{kind} of a title is specified by a \src{DockTitleVersion}. New \src{DockTitleVersions} are obtained through the \src{DockTitleManager} (there is one per \src{DockController}). Creating a new \src{DockTitleVersion} requires the calling client to provide a default \src{DockTitleFactory}.

The \textit{request} for a title is handled by a \src{DockTitleRequest}. Once a \src{DockTitleRequest} is created its method \src{request} can be called to execute the request. Clients should call \src{install} before using the request, and \src{uninstall} once the title is no longer in use. This way the \src{DockTitleRequest} will automatically be executed again if the underlying \src{DockTitleFactory} is exchanged.

Once a \src{DockTitle} is acquired it must be connected with its \src{Dockable}. Clients must call the method \src{bind( DockTitle )} of \src{Dockable}, this tells the \src{Dockable} that is has a new title. If the client no longer shows the title it calls \src{unbind( DockTitle )}.
