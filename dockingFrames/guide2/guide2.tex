...
\begin{itemize}
	...
	\item \src{V}: the generic values that have to be distributed, i.e. the class \src{Color}.
...
\designbox{This scheme allows a flexible handling of resources. On one hand the number of keys is limited and one method call is enough to change a lot things in the user interface (i.e. all background colors of titles). On the other hand clients can implement sophisticated strategies to change some properties without the need to know in detail how the property will be used.
...
No code in the framework ever assumes that some \src{DockTheme} with some special undocummented behavior is ever used. Clients can reimplement the interface \src{DockTheme} without fear to break things.

A better approach might be to extend the class \src{BasicTheme}. This class provides some default values which can easily be changed by the appropriate \src{setXZY} method.
