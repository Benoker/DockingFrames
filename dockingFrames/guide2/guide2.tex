...
\subsubsection{Finding a destination}
Several steps are necessary to find the current destination of a drag and drop operation. The search for a destination starts anew whenever the mouse is moved. The \src{DefaultDockRelocator} follows this path:
\begin{enumerate}
 ...
 \item If the user releases the mouse, and the operation ends, then the relocator calls \src{move} or \src{drop} of \src{DockStation} to complete the operation.
\end{enumerate}

\subsubsection{Reacting on events}
