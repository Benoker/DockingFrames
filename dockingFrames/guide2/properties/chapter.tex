...
There are a number of keys and the remainder of this chapter will list all of the keys that are present in version 1.0.8. If not explicitly said otherwise, then any change in the properties will have an immediate effect.
...
\begin{description}
 ...
 \property{SingleTabDecider.SINGLE\_TAB\_DECIDER}{SingleTabDecider}{\src{SingleTabDecider.NONE}}{Tells whether a single tab should be painted for some \src{Dockable}s. This makes the \src{Dockable}s look as if they were a child of a \src{StackDockStation} even if there is no such station around.}
\end{description}
