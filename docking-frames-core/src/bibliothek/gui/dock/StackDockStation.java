...
/**
 * On this station, only one of many children is visible. The other children are
 * hidden behind the visible child. There are some buttons where the user can
 * choose which child is visible. This station behaves like a
 * {@link JTabbedPane}.<br>
 ...
 */
public class StackDockStation extends AbstractDockableStation implements
	...
	public static final PropertyKey<StackDockComponentFactory> COMPONENT_FACTORY = new PropertyKey<StackDockComponentFactory>(
			"stack dock component factory");

	/**
	 * Key for setting the side at which the tabs appear in relation to the
	 * selected dockable
	 */
	public static final PropertyKey<TabPlacement> TAB_PLACEMENT = new PropertyKey<TabPlacement>(
			"stack dock station tab side",
			new ConstantPropertyFactory<TabPlacement>(
					TabPlacement.TOP_OF_DOCKABLE), true);

	/** Key for setting the {@link TabContentFilter} */
	public static final PropertyKey<TabContentFilter> TAB_CONTENT_FILTER = new PropertyKey<TabContentFilter>(
			"stack dock tab content filter");

	/**
	 * If set to <code>true</code>, then dropping a {@link Dockable} onto a
	 * {@link StackDockStation} won't change the currently selected item.<br>
	 * This property does not prevent the {@link FocusController} from changing
	 * the focus, and updating the focus may lead to changing the selected index
	 * anyway. Clients using this property might also need to implement a
	 * {@link FocusVetoListener} and add it to the {@link FocusController}.
	 */
	public static final PropertyKey<Boolean> IMMUTABLE_SELECTION_INDEX = new PropertyKey<Boolean>(
			"stack dock immutable selection index",
	...
	/**
	 * Responsible for updating a {@link DockElementRepresentative} that covers
	 * the empty areas of {@link #stackComponent}
	 */
	...
	private PropertyValue<TabContentFilter> tabContentFilter = new PropertyValue<TabContentFilter>(
			TAB_CONTENT_FILTER){
		@Override
		protected void valueChanged( TabContentFilter oldValue,
				TabContentFilter newValue ){
	...
	/**
	 * A listener added to the parent of this station. The listener ensures that
	 * the visibility-state is always correct.
	 */
	 ...
	 * 
	 * @param theme
	 *            the theme of the station, may be <code>null</code>
	 */
	 ...
	 * 
	 * @param theme
	 *            the theme of this station, can be <code>null</code>
	 * @param init
	 *            <code>true</code> if the fields of this object should be
	 *            initialized, <code>false</code> otherwise. If
	 *            <code>false</code>, then the subclass has to call
	 *            {@link #init()} exactly once.
	 */
	...
	protected void init(){
		...
		displayerFactory = new DefaultDisplayerFactoryValue(
				ThemeManager.DISPLAYER_FACTORY + ".stack", this);
		...
		stackComponentFactory = new PropertyValue<StackDockComponentFactory>(
				COMPONENT_FACTORY){
			@Override
			protected void valueChanged( StackDockComponentFactory oldValue,
		...
		tabPlacement = new PropertyValue<TabPlacement>(TAB_PLACEMENT){
			@Override
			protected void valueChanged( TabPlacement oldValue,
	...
	/**
	 * Creates the {@link StackDockComponent} which will be shown on this
	 * station if the station has more then one child.<br>
	 ...
	 * 
	 * @param placement
	 *            the side or <code>null</code> to use the default value
	 */
	...
	/**
	 * Sets the filter that tells this station how to set the content of the
	 * tabs.
	 * 
	 * @param filter
	...
	/**
	 * Gets the filter that tells this station how to set the content of the
	 * tabs.
	 * 
	...
	/**
	 * Tells whether this station should show its {@link StackDockComponent}
	 * even if it has only one child. This property may only be changed if the
	 * {@link StackDockComponent} is exchanged as well.
	 * 
	 * @return <code>true</code> if the {@link StackDockComponent} is to be
	 *         always shown
	 */
	...
	/**
	 * Sets the {@link StackDockComponent} which should be used by this station.
	 * The component is shown when this station has more then one child. Note
	 * that the <code>stackComponent</code> depends also on the property
	 * {@link #COMPONENT_FACTORY}, and will be automatically exchanged if that
	 * property changes. Clients should use
	 * {@link #setStackComponentFactory(StackDockComponentFactory)} if they want
	 * to exchange the component permanently.
	 * 
	 * @param stackComponent
	 *            the new component
	 * @throws IllegalArgumentException
	 *             if <code>stackComponent</code> is <code>null</code>
	 */
	 ...
	 * {@link StackDockComponent} for this station.
	 * 
	 * @param factory
	 *            the new factory, can be <code>null</code> if the
	...
	public void setController( DockController controller ){
		if (this.getController() != controller){
			...
			if (controller != null){
				title = controller.getDockTitleManager().getVersion(TITLE_ID,
						ControllerTitleFactory.INSTANCE);
			} else{
				title = null;
			}
	 ...
	 * {@link DefaultDisplayerFactoryValue#setDelegate(DisplayerFactory)
	 * delegate} to exchange the factory.
	 * 
	 * @return the factory
	 */
	...
	/**
	 * Gets the set of {@link DockableDisplayer displayers} used on this
	 * station.
	 * 
	...
	public boolean isVisible( Dockable dockable ){
		return isStationVisible()
				&& (dockables.dockables().size() == 1 || indexOf(dockable) == stackComponent
						.getSelectedIndex());
	}
	...
	public DockableProperty getDockableProperty( Dockable dockable,
		...
		if (strategy != null){
			placeholder = strategy.getPlaceholderFor(target == null ? dockable
					: target);
			if (placeholder != null){
	...
	/**
	 * Informs all {@link DockStationListener}s that the selected element of
	 * this station changed. This method only fires if there really is a change,
	 * hence it can be safely called multiple times.
	 */
	 ...
	 * {@link PlaceholderListItemConverter} to encode the children of this
	 * station. To be exact, the converter puts the following parameters for
	 * each {@link Dockable} into the map:
	 * <ul>
					...
					public ConvertedPlaceholderListItem convert( int index,
						...
						if (strategy != null){
							Path placeholder = strategy
									.getPlaceholderFor(dockable.getDockable());
							if (placeholder != null){
	 ...
	 * 
	 * @param placeholders
	 *            the new children and placeholders
	 * @param children
	 *            map to convert items to {@link Dockable}s
	 * @throws IllegalStateException
	 *             if there are still children on this station
	 */
	public void setPlaceholders( PlaceholderMap placeholders,
					...
					new PlaceholderListItemAdapter<Dockable, StationChildHandle>(){
						...
						public StationChildHandle convert(
							...
							DockUtilities.ensureTreeValidity(
									StackDockStation.this, dockable);
							token = DockHierarchyLock.acquireLinking(
									StackDockStation.this, dockable);

							listeners.fireDockableAdding(dockable);
							dockable.addDockableListener(listener);
							StationChildHandle handle = new StationChildHandle(
									StackDockStation.this, getDisplayers(),
									dockable, title);
	...
	public StationDropOperation prepareMove( int x, int y, int titleX,
			int titleY, boolean checkOverrideZone, Dockable dockable ){
		DockStation parent = getDockParent();
		...
		if (parent != null){
			if (checkOverrideZone
					&& parent.isInOverrideZone(x, y, this, dockable)){
				if (dockables.dockables().size() > 1){
	...
	public StationDropOperation prepareDrop( int x, int y, int titleX,
			int titleY, boolean checkOverrideZone, Dockable dockable ){
		if (dockable.getDockParent() == this){
			return prepareMove(x, y, titleX, titleY, checkOverrideZone,
					dockable);
		}
		...
		if (parent != null){
			if (checkOverrideZone
				...
				if (dockables.dockables().size() > 1){
					...
					DockTitle title = dockables.dockables().get(0)
							.getDisplayer().getTitle();
					if (title != null){
						...
						if (component.getBounds().contains(p)){
							...
							if (validate(insert, dockable)){
								return new StackDropOperation(dockable, insert,
										false);
							}
						}
					}
	 ...
	 * @param dockable
	 *            the element to drop
	 * @param autoPlaceholder
	 *            whether the {@link PlaceholderStrategy} can be invoked to
	 *            search for a matching placeholder
	 */
	public void drop( Dockable dockable, boolean autoPlaceholder ){
		Path placeholder = null;
		if (autoPlaceholder){
			PlaceholderStrategy strategy = getPlaceholderStrategy();
			placeholder = strategy == null ? null : strategy
					.getPlaceholderFor(dockable);
		}
		boolean done = false;

		if (placeholder != null){
			done = drop(dockable, new StackDockProperty(dockables.dockables()
					.size(), placeholder));
		}
		if (!done){
	 ...
	 * <code>property</code> as good as possible.
	 * 
	 * @param dockable
	 *            the new child
	 * @param property
	...
	public boolean drop( Dockable dockable, StackDockProperty property ){
		...
		if (!result){
			...
			if (index < dockables.dockables().size() && successor != null){
				DockStation station = dockables.dockables().get(index)
						.getDockable().asDockStation();
				if (station != null){
					if (station.drop(dockable, successor)){
						result = true;
					}
	...
	/**
	 * Gets the location where {@link StationDropOperation#execute()} will
	 * insert the next {@link Dockable}.
	 * 
		...
		if (property instanceof StackDockProperty){
			...
			if (index < 0)
				throw new IllegalArgumentException(
						"dockable not child of this station");
	...
	private void move( int source, int destination ){
		if (source != destination){
			...
			fireDockablesRepositioned(Math.min(source, destination),
					Math.max(source, destination));
		}
	}

	/**
	 * Tells which gap (between tabs) is chosen if the mouse has the coordinates
	 * x/y. If there is no tab at this location, a default-tab is chosen.
	 * 
	 * @param x
	 *            x-coordinate in the system of this station
	 * @param y
	 ...
	 * @param y
	 *            y-coordinate in the system of this station
	 * @return the location of a tab or <code>null</code> if no tab is under x/y
	 */
	protected Insert exactTabIndexAt( int x, int y ){
		Point point = SwingUtilities.convertPoint(panel, x, y,
	...
	public void drag( Dockable dockable ){
		if (dockable.getDockParent() != this)
			throw new IllegalArgumentException(
					"The dockable can't be dragged, it is not child of this station");

		int index = indexOf(dockable);
		if (index < 0)
			throw new IllegalArgumentException(
					"The dockable is not part of this station.");
	...
	public void replace( Dockable old, Dockable next, boolean station ){
		...
		try{
			...
			DockablePlaceholderList<StationChildHandle>.Item oldItem = dockables
					.list().get(listIndex);
			remove(index);
			add(next, index);
			DockablePlaceholderList<StationChildHandle>.Item newItem = dockables
					.list().get(listIndex);
			if (station){
	 ...
	 *            the preferred location of the new child
	 * @param placeholder
	 *            the preferred location of the new child, can be
	 *            <code>null</code>
	 */
	...
	/**
	 * Adds the contents of <code>handle</code> to the {@link #stackComponent}
	 * of this station. The new <code>handle</code> may or may not already be
	 * stored in {@link #dockables}.
	 * 
	 * @param handle
	 *            the handle to add
	 * @param index
	 *            the index where to add the handle
	 * @param size
	 *            the current amount of children of the panel, must be either
	 *            the size of {@link #dockables} if <code>handle</code> is not
	 *            yet stored or the size of {@link #dockables}-1 is
	 *            <code>handle</code> already is stored.
	 */
		...
		if (size == 0 && !singleTabStackDockComponent()){
			...
			if (size == 1 && !singleTabStackDockComponent()){
				...
				if (list.get(0) == handle){
					if (list.size() != 2){
						throw new IllegalStateException(
								"handle is stored, size is 1, but number of known dockables is not 2");
					}
					handle = list.get(1);
					index = 1;
				}
	...
	private void insertTab( DockableDisplayer displayer, int index ){
		...
		stackComponent.insertTab(title, icon, displayer.getComponent(),
				dockable, index);
	 ...
	 * 
	 * @param index
	 *            the location of the child which will be removed
	 */
		...
		try{
			...
			if (dockables.dockables().size() == 1){
				if (singleTabStackDockComponent()){
					...
					panel.remove(dockables.dockables().get(0).getDisplayer()
							.getComponent());
				}
				dockables.remove(0);
				panel.repaint();
				...
				panel.add(dockables.dockables().get(0).getDisplayer()
						.getComponent());
			} else{
				dockables.remove(index);
	...
	/**
	 * Updates the {@link #getConfigurableDisplayerHints() displayer hints} of
	 * this station.
	 */
	...
	private void updateContent( int index ){
		if (index >= 0
				&& (getDockableCount() > 1 || singleTabStackDockComponent())){
			Dockable dockable = getDockable(index);
			TabContentFilter filter = getTabContentFilter();

			TabContent content = new TabContent(dockable.getTitleIcon(),
					dockable.getTitleText(), dockable.getTitleToolTip());
			if (filter != null){
				content = filter.filter(content, this, dockable);
			}
			if (content == null){
	...
	/**
	 * A listener for the parent of this station. This listener will fire events
	 * if the visibility-state of this station changes.<br>
	 * This listener is also added to the
	 * {@link StackDockStation#getStackComponent() stack-component} of the
	 * station, and ensures that the visible child has the focus.
	 * 
	 * @author Benjamin Sigg
	 */
	private class VisibleListener extends DockStationAdapter implements
			ChangeListener{
		@Override
	...
	/**
	 * This listener is added to the children of the station. Whenever the icon
	 * or the title-text of a child changes, the listener will inform the
	 * {@link StackDockStation#getStackComponent() stack-component} about the
	 * change.
	 * 
	 * @author Benjamin Sigg
	 */
	...
	protected class Background extends SecureContainer{
		...
		protected void paintOverlay( Graphics g ){
			...
			if (insert != null && dockables.dockables().size() > 1){
				...
				if (insert.tab >= 0
						&& insert.tab < stackComponent.getTabCount())
				...
				if (bounds != null){
					...
					if (insert.right){
						insertionLine(
								bounds,
								insert.tab + 1 < stackComponent.getTabCount() ? stackComponent
										.getBoundsAt(insert.tab + 1) : null, a,
								b, true);
					} else{
						insertionLine(
								insert.tab > 0 ? stackComponent.getBoundsAt(insert.tab - 1)
	 ...
	 * 
	 * @param left
	 *            the bounds of the tab left to the line, might be
	 *            <code>null</code> if <code>leftImportant</code> is
	 *            <code>false</code>.
	 * @param right
	 *            the bounds of the tab right to the line, might be
	 *            <code>null</code> if <code>leftImportant</code> is
	 *            <code>true</code>.
	 * @param a
	 *            the first point of the line, should be used as output of this
	 *            method
	 * @param b
	 *            the second point of the line, should be used as output of this
	 *            method
	 * @param leftImportant
	 *            <code>true</code> if the mouse is over the left tab,
	 *            <code>false</code> if the mouse is over the right tab.
	 */
	protected void insertionLine( Rectangle left, Rectangle right, Point a,
			Point b, boolean leftImportant ){
		if (tabPlacement.getValue().isHorizontal()){
			if (left != null && right != null){
				int top = Math.max(left.y, right.y);
				int bottom = Math.min(left.y + left.height, right.y
						+ right.height);
			...
			if (left != null && right != null){
				...
				int bottom = Math.min(left.x + left.width, right.x
						+ right.width);
	...
	protected class StackDropOperation implements StationDropOperation{
		 ...
		 * 
		 * @param dropping
		 *            the item that is about to be dropped
		 * @param insert
		 *            the new location of {@link #dropping}
		 * @param move
		 *            whether this is a move operation
		 */
		public StackDropOperation( Dockable dropping, Insert insert,
				boolean move ){
	 ...
	 * @author Benjamin Sigg
	 */
	private class PanelBackground extends BackgroundAlgorithm implements
			StationBackgroundComponent{
		/**
		 * Creates a new algorithm.
		 */
		public PanelBackground(){
			super(StationBackgroundComponent.KIND,
					ThemeManager.BACKGROUND_PAINT + ".station.stack");
		}
