...
\subsubsection{DockableFocusListener, DockStationListener}
\begin{description}
 \short The methods of \src{DockableFocusListner} and \src{DockStationListener}\\have more parameters. Methods which indicate a change, like focusing a new \src{Dockable}, now also tell what was the old value of the changed property.
 \why No need for listeners to store the old values.
 \clients Must carefully update all classes and interfaces that implement \\\src{DockableFocusListener} or \src{DockStationListener}. Work should be easier when using the \src{@Override} annotation. Be especially careful not to mix up the new arguments with the old ones.
\end{description}
...
\begin{description}
 \short The \src{ComponentHierarchyObserver} includes more \src{Components} in its search.
...
\begin{description}
 ...
 \why Allows to be more flexible with colors, allows the use of Nimbus.
\end{description}
