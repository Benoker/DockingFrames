...
Every \src{AbstractCDockable} has the method \src{setResizeLocked}. If the size is locked through this method than any station will try not to change the size of the \src{dockable}. There are also methods to lock only width or height \linebreak (\src{setResizeLockedHorizontally} and \src{setResizeLockedVertically}).
...
When maximizing a \src{CDockable} first the global \src{CMaximizeBehavior} is asked for the real element to maximize. If for example a \src{dockable} is part of a whole stack of \src{dockable}s, then the maximize-behavior might decide to maximize the whole stack instead of the single \src{dockable}. In a second step the tree of stations and \src{dockable}s is traversed upwards until a \src{MaximizeArea} is found, the element is then shown on this area. If no \src{MaximizeArea} can be found in the ancestors, then a default area is taken.
