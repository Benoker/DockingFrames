...
/**
 * The {@link AnimationScheduler} is responsible for repeatedly invoking the animation
 * code. The {@link AnimationScheduler} may perform optimations, like coalescing calls.<br>
 * Usually a scheduler should call the animations in the EDT (EventDispatcherThread).
 * @author Benjamin Sigg
 */
public interface AnimationScheduler {
	/**
	 * Calls {@link AnimationSchedulable#step(AnimationScheduler, int)} with some reasonable
	 * delay, usually around 20 milliseconds.
	 * @param job the animation to call
	 * @see #step(AnimationSchedulable, int)
	 */
	public void step( AnimationSchedulable job );
	
	/**
	 * Calls {@link AnimationSchedulable#step(AnimationScheduler, int)} with a delay of about
	 ...
	 * execution of {@link AnimationSchedulable#step(AnimationScheduler, int)}, then the
